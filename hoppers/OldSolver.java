public class OldSolver implements Runnable
{
    private Board beginningState;
    private Square startSquare;

    OldSolver(Board gameState, int frogIndex)
    {
    ...
    public void run()
    {
        for(int[] coords = {0,0}; coords[0] < 5; coords[0]++)
        {
            ...
            {
                if(movePossible(startSquare, beginningState.getSquare(coords[0], coords[1]), beginningState))
                {
                    beginningState.moveFrog(startSquare, getTakenSquare(beginningState, startSquare.getCoordinates(), coords), beginningState.getSquare(coords));
